<template>
  <div class="page" @scroll='getMore'>
    ...
    <div class='section' v-if='docList.length > 0' ref='secList'>
      <div class='doctor'>
        <ul>
          <li v-for='(item,index) in docList' v-if='showType === "CONSULT_FREE" ? item.volunteers > 0 : true' :key='item.doctorId' :data-code='item.code' @click='jumpTo(item.doctorId)'>
 ...
 beforeRouteLeave(to, from, next) {
  ...
  methods: {
    ...
    getFirstInfo(type) {
      ...
      let url = `${this.$conf.url}/internethospital/api/doctors?page-index=${this.pages}&page-size=10`;
    ...
    quickSearch(e) {
      ...
      if (name === this.searchType) {
        ...
        if (name === 'hospital') {
          ...
          this.$axios.get(urlH, { headers: this.$store.state.header }).then(res => {
            console.log(res);
              let defaultHospital = {
                id: 'default',
