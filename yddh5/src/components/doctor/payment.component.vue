<template>
  <div class="page">
    <div class="srollder">
      ...
      <div class='section' v-if='docInfo'>
        <div class='nurseInfo'>
          <div class='infoBox'>
            ...
            <div class='left'>
              ...
              <div><span>￥{{docInfo.services[0].price ? docInfo.services[0].price : 0}}/次</span></div>
            </div>
          </div>
        </div>
        <div class='middel-part'>
          ...
          <div class='price'>
            ...
            <span class='right'>￥{{docInfo.services[0].price ? docInfo.services[0].price : 0}}</span>
          </div>
        </div>
        ...
        <div class='docotr_notice'>
          <span>问诊须知 <br></span>
          <span>1. 提问成功后，若医生在24小时内未回复，订单费用将在5个工作日内退回；<br>
    2. 医生回复后，单次服务有效时间为24小时；<br>
    ...
    <!--    <div class="getUserInfo" :class="getUserInfo ? 'show' : ''">
      <div>
        <div>
          <span>我们希望获得您的个人信息，\n并注册个人账号</span>
        </div>
        <div>
          <button @click='notNow'>先看看</button>
          <button open-type='getUserInfo' bindgetuserinfo='makeChoise'>好的</button>
        </div>
      </div>
    </div> -->
    <div class="clause" @click='closeAlert' :class="clauseShow ? 'show' : ''">
      <div>
        <div>
          <span>互联网医院网络门诊\n问诊须知</span>
...
export default {
  ...
  beforeRouteLeave(to, from, next) {
    ...
    let chat = /chat/g
    let doctor = /doctor/g
    if (doctor.exec(to.path) || chat.exec(to.path) || payment.exec(to.path)) {
  ...
  methods: {
    ...
    getFirstInfo(id) {
      const me = this;
      const url = `${this.$conf.url}/internethospital/api/doctors/${id}`;
    ...
    getOrderInfo(id) {
      const me = this;
      const url = `${this.$conf.url}/internethospital/api/orders/consultations?order-header-id=${id}&page-index=1&page-size=10`;
      this.$axios.get(url, { headers: this.$store.state.header }).then(res => {
          let content = res.data.content[0];
          console.log(content);
          if (!content.doctorAvatar) {
          ...
          let docInfo = {
            doctorAvatar: content.doctorAvatar,
      ...
      if (this.hadCoupon) {
        ...
        this.$axios.post(url, {}, { headers: this.$store.state.header }).then(res => {
          let enableCoupon = res.data.content.filter(c =>
            c.status === 'AVAILABLE' &&
            (c.departmentId ?
              c.departmentId === this.data.docInfo.departmentId :
              c.organizationId ?
              c.organizationId === this.data.docInfo.organizationId :
              true
            )
          );
    ...
    confirmPrice(couponProductId) {
      ...
      let data = {
        ...
        doctorId: this.docInfo.doctorId,
        departmentId: this.docInfo.departmentId,
        orderTypeCode: this.docInfo.services[0].code,
        organizationId: this.docInfo.organizationId,
        productId: this.docInfo.services[0].id
      }
      this.$axios.post(url, data, { headers: this.$store.state.header }).then(res => {
        this.amount = res.data
      }).catch(err => {
        this.notice_show = err
        setTimeout(() => {
          this.notice_show = ''
        }, 2000)
    ...
    gotToPay() {
      const me = this;
      console.log(this.orderHeaderId)
      if (this.orderHeaderId) {
        if (me.docInfo.services[0].free || (me.amount === 0)) {
        ...
        this.$axios.post(url, data, { headers: this.$store.state.header }).then(res => {
          console.log(res);
            if (me.docInfo.services[0].free || (me.amount === 0)) {
              ...
              me.disciount(res.data.orderHeaderId)
              this.notice_show = '创建订单成功'
              setTimeout(() => {
                this.notice_show = ''
              }, 2000)
    ...
    disciount(orderHeaderId) {
      ...
      this.$axios.get(urlperson, { headers: this.$store.state.header }).then(res => {
        console.log(res)
        setTimeout(() => {
          this.$router.push(`/chat/${res.data.content[0].orderHeaderId}`)
        }, 1500);
      }).catch(err => {
     this.notice_show = err.response.data.message
          setTimeout(() => {
    ...
    pay(orderHeaderId) {
      ...
      //           // setTimeout(function () {
      //           //   wx.navigateTo({
      //           //     url = `/pages/chat/chat/chat?doctorId=${res.content.doctorId}&doctorAvatar=${res.content.doctorAvatar}&orderId=${res.content.orderHeaderId}`
      //           //   })
      //           // }, 1500);
      //         },
      //         'fail': err => {
      ...
      //       wx.showToast({
      //         icon: 'none',
      //         title: `获取支付信息失败${res.data.message}`,
      //         duration: 2000
      //       })
      //       setTimeout(() => {
      //         wx.navigateBack();
